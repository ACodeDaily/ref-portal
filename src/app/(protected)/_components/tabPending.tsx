...
const PendingRequestsPage = ({
    ...
    const updateFormData = (updatedFormData: form) => {
        let tempPending: Array<form> = []
        let tempAccepted: Array<form> = acceptedForms;
        let tempRejected: Array<form> = rejectedForms;

        for (let i = 0; i < pendingForms.length; i++) {
            if (pendingForms[i].id === updatedFormData.id) {
                if (updatedFormData.status === Status.ACCEPTED) {
                    tempAccepted.push(updatedFormData);
                }
                else if (updatedFormData.status === Status.REJECTED) {
                ...
                tempPending.push(pendingForms[i]);
            }
        }
        tempAccepted = tempAccepted.sort((a, b) => (b.member.codeForcesRating || 0) - (a.member.codeForcesRating || 0));
        tempRejected = tempRejected.sort((a, b) => (b.member.codeForcesRating || 0) - (a.member.codeForcesRating || 0));
