    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await fetch('/api/form/pending'); // Adjust the API endpoint based on your actual setup
                ...
                for (const member of sortedMembers) {
                    if (member.forms && Array.isArray(member.forms)) {
                        for (const form of member.forms) {
                            if (role === 'REFERRER' && form.organization === user?.organization && form.status === "PENDING") {
                                temp.push([member, form]);
                            }
                            else if ((role === 'ADMIN' || role === 'MOD') && form.status === "PENDING") {
                                temp.push([member, form]);
                            }
                        }
                    }
            ...
            <RoleGate allowedRole={[UserRole.ADMIN, UserRole.MOD, UserRole.REFERRER]}>
                ...
                <div className="w-[90%] flex flex-col justify-center">
                    {members.map((member, i) => {
                        ...
                        (pathname === '/form/accepted' || pathname === '/form/rejected') ? '' :
                        member[1].status === Status.ACCEPTED
                            ? 'bg-green-200'
                            : member[1].status === Status.REJECTED
                                ? 'bg-red-300'
                                : 'bg-gray-100';
                        return (
                            <Card key={i} className={`w-[100%] mb-3 flex flex-col justify-center ${statusColorClass}`}>
