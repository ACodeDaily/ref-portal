    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await fetch('/api/form/rejected'); // Adjust the API endpoint based on your actual setup
                ...
                for (const member of sortedMembers) {
                    if (member.forms && Array.isArray(member.forms)) {
                        for (const form of member.forms) {
                            if (role === 'REFERRER' && form.organization === user?.organization && form.status === "REJECTED") {
                                temp.push([member, form]);
                            }
                            else if ((role === 'ADMIN' || role === 'MOD') && form.status === "REJECTED") {
                                temp.push([member, form]);
                            }
                        }
                    }
    ...
    const updateFormData = (updatedFormData: form) => {
        ...
        for (let i = 0; i < members.length; i++) {
            if (members[i][1].id === updatedFormData.id) {
                if (updatedFormData.status === Status.REJECTED) {
